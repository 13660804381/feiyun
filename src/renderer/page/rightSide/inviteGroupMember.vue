...
import { getFriendList, joinGroupMember } from "../../request/api";
export default {
  data() {
    return {
      selectList: [],
        ...
        if (res) {
          ...
          for (let i = 0; i < list.length; i++) {
            for (let j = 0; j < this.memberList.length; j++) {
              if (list[i].id == this.memberList[j].id) {
