...
export default {
  ...
  created() {
    ...
    for (let i = 0; i < this.friendListData.length; i++) {
      if (this.friendListData[i].isAdmin == 1) {
        this.friendListData[i].selected = true;
        if (this.friendListData[i].isCreator !== 1) {
          this.selectList.push(this.friendListData[i]);
        }
      }
    }
